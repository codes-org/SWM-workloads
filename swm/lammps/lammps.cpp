...
LAMMPS_SWM::LAMMPS_SWM(
    ...
    AppBaseSWMUserCode(user_if, cfg, "lammps"),
    x_rep(cfg.get<uint32_t>("num_x_replicas", 1)),
    y_rep(cfg.get<uint32_t>("num_y_replicas", 1)),
    z_rep(cfg.get<uint32_t>("num_z_replicas", 1)),
    num_timesteps(cfg.get<uint32_t>("num_time_steps", 100)),
    router_freq(cfg.get<double>("router_freq", 800e6)),
    cpu_freq(cfg.get<double>("cpu_freq", 1.2e9)),
    cpu_sim_speedup(cfg.get<double>("cpu_sim_speedup", 1.0))
{
